...
describe('【MES】制造执行系统模块 (41 APIs)', () => {
  ...
  describe('工单管理 (7)', () => {
    ...
    it('MES-003: POST /api/v1/mes/work-orders - 创建工单', async () => {
      const resp = await post(API + '/mes/work-orders', {
        ...
        plannedQty: 100, uomId: testData.uom?.id , status: 'RELEASED'
      })
      expect([200, 201, 204]).toContain(resp.status)
    })
  ...
  describe('报工 (3)', () => {
    it('MES-014: POST /api/v1/mes/work-orders/:id/report - 报工', async () => {
      ...
      const woId = woResp.body?.data?.items?.[0]?.id || woResp.body?.data?.[0]?.id 
...
describe('【PLM】产品生命周期管理模块 (63 APIs)', () => {
  ...
  describe('BOM版本管理 (3)', () => {
    ...
    it('PLM-053: POST /api/v1/plm/boms - 创建BOM', async () => {
      ...
      const materialId = testData.materials?.[0]?.id 
...
describe('【ERP】企业资源计划模块 (88 APIs)', () => {
  ...
  describe('标准成本 (3)', () => {
    ...
    it('ERP-069: POST /api/v1/erp/standard-costs - 创建标准成本', async () => {
      const resp = await post(API + '/erp/standard-costs', { materialId: testData.material?.id  })
      expect([200, 201, 204]).toContain(resp.status)
    })
...
describe('【EAM】设备管理系统模块 (56 APIs)', () => {
  ...
  describe('维保计划 (4)', () => {
    ...
    it('EAM-019: POST /api/v1/eam/maintenance/plans - 创建维保计划', async () => {
      const resp = await post(API + '/eam/maintenance/plans', { equipmentId: testData.equipment?.id  })
      expect([200, 201, 204]).toContain(resp.status)
    })
  ...
  describe('点检润滑 (6)', () => {
    it('EAM-022: POST /api/v1/eam/inspection-records - 点检记录', async () => {
      const resp = await post(API + '/eam/inspection-records', { equipmentId: testData.equipment?.id  })
      expect([200, 201, 204]).toContain(resp.status)
    })
    ...
    it('EAM-025: POST /api/v1/eam/lubrication-records - 创建润滑记录', async () => {
      const resp = await post(API + '/eam/lubrication-records', { equipmentId: testData.equipment?.id  })
      expect([200, 201, 204]).toContain(resp.status)
    })
...
describe('【BASE】基础模块 (13 APIs)', () => {
  ...
  describe('批次管理 (4)', () => {
    ...
    it('BASE-010: POST /api/v1/base/batches - 创建批次', async () => {
      const resp = await post(API + '/base/batches', { batchNo: `BAT_${testData.ts}`, materialId: testData.material?.id , initialQty: 100, currentQty: 100, uomId: '1', sourceType: 'PURCHASE' })
      expect([200, 201, 204]).toContain(resp.status)
    })
  ...
  describe('委外订单 (8)', () => {
    ...
    it('OUT-002: POST /outsourcing/orders - 创建委外订单', async () => {
      const resp = await post(BASE_URL + '/outsourcing/orders', {
        supplierId: '1', materialId: testData.material?.id , plannedQty: 100, unitPrice: 10,
        processName: `OUT_${testData.ts}`, issueWarehouseId: testData.warehouse?.id , plannedDelivery: '2026-05-20'
      })
      expect([200, 201, 204]).toContain(resp.status)
    })
  ...
  describe('追溯批次 (5)', () => {
    ...
    it('TRC-004: POST /traceability/batches/manual - 手动创建批次', async () => {
      const resp = await post(BASE_URL + '/traceability/batches/manual', {
        materialId: testData.material?.id , materialCode: 'MAT001', materialName: 'Test', batchNo: `TRC_${testData.ts}`
      })
      expect([200, 201, 204]).toContain(resp.status)
    })
